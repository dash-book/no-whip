...
let users = [
  { id: ulid(), username: 'juan_gomez', email: 'juan.gomez@example.com', password: 'password1', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'maria_lopez', email: 'maria.lopez@example.com', password: 'password2', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'carlos_perez', email: 'carlos.perez@example.com', password: 'password3', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'laura_garcia', email: 'laura.garcia@example.com', password: 'password4', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'antonio_fernandez', email: 'antonio.fernandez@example.com', password: 'password5', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'lucia_martinez', email: 'lucia.martinez@example.com', password: 'password6', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'javier_rodriguez', email: 'javier.rodriguez@example.com', password: 'password7', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'sara_sanchez', email: 'sara.sanchez@example.com', password: 'password8', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'david_ramos', email: 'david.ramos@example.com', password: 'password9', roles: ['EMPLOYEE'], created_at: new Date() },
  { id: ulid(), username: 'sofia_gutierrez', email: 'sofia.gutierrez@example.com', password: 'password10', roles: ['MANAGER'], created_at: new Date() }
];
...
app.get('/api/tracking/:employeeId', (req, res) => {
  ...
  if (employeeTracking.length === 0) {
    return res.status(404).json({ message: 'No tracking data found for this employee' });
  }
...
app.get('/api/tracking/:employeeId/:date', (req, res) => {
  ...
  const record = tracking.find(t => t.employeeId === employeeId && t.date.toDateString() === trackingDate.toDateString());

  if (!record) {
    return res.status(404).json({ message: 'No tracking record found for this date' });
  }
  res.json(record);
});
...
app.get('/api/audit', (req, res) => {
  ...
  if (filteredTracking.length === 0) {
    return res.status(404).json({ message: 'No tracking data found for the selected dates' });
  }
...
app.get('/api/working', (req, res) => {
  ...
  const workingUsers = tracking
    ...
    .map(t => {
      ...
      return user ? { id: user.id, username: user.username, email: user.email, roles: user.roles } : null;
    })
    .filter(Boolean); // Remover posibles null

  if (workingUsers.length === 0) {
    return res.status(404).json({ message: 'No users were working at the specified time.' });
  }
