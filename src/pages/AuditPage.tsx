...
const AuditPage: React.FC = () => {
  ...
  const handleStartAudit = () => {
    ...
    message.warning("The audit has started. Employees will be notified that their data is being requested.");
  };
  ...
  const handleDownloadAudit = async (values: any) => {
    ...
    try {
      ...
      const fileName = `audit_report_${moment().format('YYYY-MM-DD_HH-mm-ss')}.csv`;

      new CsvBuilder(fileName)
        .setColumns(["Employee Name", "Date", "Entry Time", "Exit Time"])
        .addRows(csvData.map((item: CsvRecord) => [item.employeeId, item.date, item.entryTime, item.exitTime]))
        .exportFile();
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <p>
        By starting the audit, you will be able to access employee tracking data, but employees will be notified that their working hours are being reviewed.
      </p>

      {!isAuditStarted && (
        <Button type="primary" onClick={handleStartAudit}>
          Start Audit
        </Button>
      )}

      {isAuditStarted && (
        <Form form={form} onFinish={handleDownloadAudit} layout="vertical" style={{ marginTop: '20px' }}>
          <Form.Item
            name="range"
